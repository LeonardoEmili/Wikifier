...
async def main():
    ...
    # This seed url initializes the list of urls used to generate random pages
    seed_url = ["https://en.wikipedia.org/wiki/Cat"]
    generated_urls = set(seed_url)
    visited_urls = set()        # Used to keep track of already visited urls
    for i in range(1, len(sys.argv)):
        if sys.argv[i].startswith("-") or sys.argv[i] == str(options.NUM):
            continue        # These are options and then not parsed here
        output_no, new_urls = await scrape_website(sys.argv[i], options, output_no, visited_urls)
        # Update the value of generated_urls by adding new_urls but clearing it from already visited_urls
    ...
    while options.NUM > 0:      # In the case the user asked for NUM more pages to be parsed they're randomically selected using 'generated_urls' list
        ...
        # Random sample without replacing
        for url in random.sample(generated_urls, sample_size):
...
async def scrape_website(url, options, output_no, visited_urls):
    # Check if the given link is owned by Wikipedia
    wikipedia_url_re = re.compile('^(https?://)?([A-z]{2}\.)?wikipedia\.org')
    print(url)
    if (not wikipedia_url_re.match(url)):
        print('The url {} cannot be processed. Remember that only Wikipedia\'s links are allowed (e.g. https://en.wikipedia.org/wiki/Cat)'.format(url), file=sys.stderr)
        return (output_no, set())
    # Capture using regex url ending with #some_id
    hashtag_ref_re = re.compile('#.*$')
    # Remove the trailing references (useless in this script)
    url = re.sub(hashtag_ref_re, '', url)
    try:
        # Main stuff going here, it extracts content text from page
        data = parse_website(url)
        visited_urls.add(url)
    except Exception as e:
        print("Runtime error: {} cannot be processed due to some errors, it will be ignored.".format(url))
        # Empty set and same output_no are returned in case of error
        return (output_no, set())
...
async def resolve_redirects(data):
    # Take the value of each pair {key : value} that is None in case of plain text as key
    urls = [word_dict[list(word_dict.keys())[0]] for word_dict in data]
    async with ClientSession(loop=loop) as session:
        result = await fetch_all(session, urls)
        # We want to create a result list which has the same length as data for faster item retriving purposes
...
async def fetch_all(session, urls):
    # Wait for each url to be fetched
...
async def fetch(session, source_url):
    if source_url == None:
        # In this case it would try to parse plain text, then a placeholder is returned
        return {None, None}
    async with session.get(source_url) as response:
        # Capture using regex what's inside 'href' attribute
        re_match = re.compile('<link rel="canonical" href="(.*)"/>')
...
def parse_website(url):
    ...
    # Status code in range 400-500 then some errors occurred
    ...
    # This list will store all the pairs as the form: (link_text : link_url) or (text: None)
    ...
    for paragraph in soup.find_all("p"):
        ...
        for el in paragraph.contents:
            if (el.name == 'a'):
                ...
                # Join between base 'url' and 'link_url' which is relative to it
                link_url = urljoin(url, el.get('href'))
                text_content.append(el.get_text(), link_url, new_element)
            elif isinstance(el.string, Tag) and el.get_text().strip():
                # Add entry by checking the value of 'new_element' flag
                text_content.append(el.get_text(), None, new_element)
                new_element = False
            # Add entry by checking the value of 'new_element' flag
...
def clear_tags(soup):
    ...
    for u in soup.find_all(invalid_tags):
        if u.name == "a" and (u.get('href') == None or u.get('href')[0] == '#'):
            ...
            u.extract()
    upper_label = soup.find(
        "div", {"class": "hatnote navigation-not-searchable"})
    if upper_label is not None:
        # Remove the upper label where it says 'page x redirects here ... etc ...'
        upper_label.extract()
...
def get_options(argv):
    ...
    try:
        ...
        exit("{}: missing mandatory arguments\nTry 'python3 {} -h' or 'python3 {} --help' for more information.".format(
            sys.argv[0], sys.argv[0], sys.argv[0]))
    if options.NUM == None:
        options.NUM = 0     # Restore its default value
    return options
